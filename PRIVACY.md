...
Website analytics are **enabled as of September 17, 2026**, using the owner's public counting endpoint at `https://mihirbhadak.goatcounter.com/count`. The bundled integration sends GoatCounter page visits, coarse referral origins/campaign labels and allowlisted clicks. The provider can derive approximate location/browser summaries from these requests. The website footer discloses the service and offers **Turn off website metrics**; this preference persists across visits in that browser. Do Not Track and Global Privacy Control also prevent analytics requests. The only browser preference stored by the integration is the visitor's website-metrics opt-out; no persistent client-generated tracking ID is added. The provider receives connection metadata and applies its own [privacy policy](https://www.goatcounter.com/help/privacy). Form contents, captured API traffic, arbitrary URL query strings and credentials are not part of the integration's event payloads. No third-party analytics script is loaded. Analytics failures never gate website functionality. Local development visits are excluded.

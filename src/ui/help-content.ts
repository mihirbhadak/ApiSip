export const helpTopics = [
  {
    ...
    steps: [
      ...
      'Response baseline builds a named structural check from the captured response or a pasted JSON sample. Preview paths/types, then Use this baseline. Values are not saved or compared. Ignore exact JSON Pointer branches and optionally allow additional fields. Arrays use exact indices; item counts can change the result. This observed example is not a formal API specification.',
  ...
  },
  {
    ...
    steps: [
      ...
      'Automatic prefers the original browser tab when available. Browser uses eligible cookies and CORS rules from the source tab. Extension uses granted host access and omits ambient cookies by default. Neither reads page localStorage or runs page login code.',
      'Browser cookies is separate from the header eyes. Hiding every header does not remove Chrome-managed cookies. Choose Do not send browser cookies to test without them; also exclude Authorization/API-key headers and credential fields for an anonymous test.',
      'For cookie login from Extension, explicitly choose Use eligible browser cookies. This choice is saved with the draft and bound to the target origin; changed origins require choosing again. Redirects are blocked in this mode. Cookie rules/partitioning still apply, and CSRF fields may still be required. Browser is the closest match to a page login.',
      'A 401/403 can mean a missing or expired credential, CSRF protection, or account permissions. ApiSip adds troubleshooting guidance without retrying. Timed runs and Test lab continue to omit ambient cookies; use explicit test-token headers or a login step with token extraction.',

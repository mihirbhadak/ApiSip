...
## 0.3.0 replay authentication and response baselines — September 17, 2026

The editor now separates execution context, browser-cookie policy and captured header inclusion. Cookie choices are explicit, saved with the draft and origin-bound. Extension cookie inclusion rejects redirects. Test lab adds named response-shape baselines and follows appearance changes without losing edits. [REPLAY-CONTEXT.md](REPLAY-CONTEXT.md) explains authentication; [TEST-LAB.md](TEST-LAB.md) explains baseline semantics and limits.

- **304 unit/component/integration tests across 34 files passed**, plus lint, strict TypeScript, formatting and the production build. `npm audit` reported zero vulnerabilities. Following final baseline boundary/preview hardening, **45 focused tests** and lint/typecheck/build passed again.
- **All 33 actual-extension Chrome E2E scenarios passed together** (6.2 minutes). The final build then passed both new authentication/baseline scenarios again. The suite continues to cover real capture/badges, current/all-tab scope, headers/bodies, filters, editor tabs, both replay contexts, clipboard/export/import, persistence, permissions, fresh installation, browser/worker restarts, timed runs and accessibility.
- The authentication test signs in through a real local HTTP-only cookie, verifies JavaScript cannot read it, captures an API call, hides all captured headers and verifies **Browser 200 / Extension default 401 / Extension explicitly including cookies 200 / Browser omitting cookies 401**. Token-only extension replay also succeeds with cookies omitted. Draft reload preserves the choice; changed origins refuse to send and credentialed extension redirects fail. Chrome decides cookie eligibility; this fixture does not certify arbitrary website logins.
- The baseline test reads an actual captured response, persists its shape, passes against unchanged types, detects a number-to-string change and an extra field, configures ignored paths/additional fields, reloads and passes. Exported v2 definitions preserve checks without sample values. Unit regressions cover escaped/hostile/space-containing paths, null, exact array indices, removed fields, subtree boundaries, malformed imports, UTF-8 byte limits, depth/field limits, truncated bodies and clearing stale previews after invalid input.
- New views pass automated light/dark accessibility checks. Editor layout has no overflow at 640 px; screenshots were visually inspected. The Test lab theme regression was fixed in the actual settings subscription, without resetting workspace or unsaved definitions.
- **23 website browser tests and 3 website-tool tests passed**, covering release URLs, local assets, responsive/accessibility checks at 1440/768/390/320 px, analytics privacy controls, download feedback, installation guidance and links.
- Local performance: **10,000 real timed requests in 60,014.5 ms, zero missed starts**, approximate P95 12.48 ms, maximum start delay 28.34 ms, 32,659-byte report and 61 ms inspector interaction. Inspector JS heap was 15,020,508 bytes after 1,000 requests and 15,346,756 after 8,000. Sampled CPU time was 57.88 CPU-seconds across surviving isolated Chrome processes over 60.87 seconds; this is not whole-computer CPU or total browser memory. The 10,000-capture search completed in 838 ms. These are local Windows fixture measurements, not rate/resource guarantees for other systems or APIs.

Final build: `2026-09-17T17:38:02.910Z`. ZIP: **32 files, 1,298,198 bytes**. SHA-256: `253146708b5209dfdf01cc36a119ccb93b6205eccb9623e307b717c506f0f4bf`. Packaging verified every archived byte, both manifest layouts and **121 source-map source entries** against the final code. An extracted copy of that exact ZIP was also checked through the fresh Chrome installation test.

No added extension permissions or runtime dependencies. Single-replay cookie inclusion does not carry over to timed runs/suites. Live AI/MCP, mocking, OpenAPI/schema import and CI export remain planned in [ROADMAP.md](ROADMAP.md); they are not claimed as implemented or tested.
